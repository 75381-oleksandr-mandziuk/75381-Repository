...
if (isValid) {
        // ZADANIE 8: Komunikacja z własnym backendem na Railway
        ...
        const railwayUrl = 'https://cv-backend-75381-production.up.railway.app/api/kontakt';
        ...
        fetch(railwayUrl, {
            ...
            body: JSON.stringify(daneZFormularza)
        })
        .then(response => {
        ...
        .then(data => {
            alert('Sukces! Wiadomość trafiła na nasz serwer Railway.');
            kontaktForm.reset();
        })
        .catch(error => {
            console.error('Błąd:', error);
            alert('Wystąpił problem z połączeniem z backendem.');
        });
    }
    });
}
